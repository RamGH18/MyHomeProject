...
// Fixed "Current Location" for prototype (Mountain View Center)
...
function performSearch(category, zipcode) {
...
function viewDetails(vendorId, zipcode) {
    ...
    requestForm.classList.add('hidden'); modalBody.classList.remove('hidden');
    ...
    modalBody.innerHTML = `
        ...
        <div class="group-buy-container">
            ...
            <div class="progress-track"><div class="progress-fill" style="width: ${progressPercent}%"></div>
                ${gb.tiers.map(t => `<div class="tier-marker" style="left: ${(t.count/maxTiers)*100}%"></div><div class="tier-label" style="left: ${(t.count/maxTiers)*100}%">${t.discount}%</div>`).join('')}
            </div>
            <p style="font-size:12px; color:var(--ios-gray); margin-top:20px; text-align:center;">
                ${gb.current < maxTiers ? `Only <strong>${nextTier.count - gb.current} more</strong> neighbors for ${nextTier.discount}% OFF!` : 'MAX DISCOUNT ACHIEVED!'}
            </p>
        ...
        ` : `
            <div style="background:#f0f7ff; padding:15px; border-radius:12px; border:1px solid #d0e7ff;">
                <h4 style="margin:0; color:var(--ios-blue);">Be a First Mover!</h4>
                <p style="font-size:14px;">No neighbors joined yet. Start this project to kick off the group deal for your street!</p>
                <button class="ios-btn" style="width:100%; margin-top:10px;" onclick="showRequestForm()">Start Neighborhood Deal</button>
            </div>
        `}
    `;
...
document.getElementById('submitRequestBtn').onclick = () => {
    ...
    alert(`Success! You've started the neighborhood group!`);
...
function simulateVendorResponse(request) {
    setTimeout(() => {
        const vendorReplyText = `Thanks for starting the group! I'll be in your neighborhood soon. As more neighbors join, your price will automatically drop further.`;
        inboxMessages.unshift({ id: Date.now(), from: request.vendorName, subject: `Neighborhood Deal Started`, body: vendorReplyText, date: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) });
...
function sendMessage() {
    ...
    input.value = '';
    renderChatHistory();
    setTimeout(() => { chatHistory[currentVendorChat].push({ from: 'vendor', text: "Got it! Checking my schedule.", date: new Date().toLocaleTimeString() }); renderChatHistory(); }, 2000);
}

function confirmAccept(vendorName) {
    const req = userRequests.find(r => r.vendorName === vendorName);
    if (req) req.status = 'Scheduled';
    alert(`Success! Opening Calendar...`);
...
function renderMyProjects() {
    ...
    userRequests.forEach(req => {
        const card = document.createElement('div'); card.className = 'vendor-card'; card.style.flexDirection = 'column'; card.style.alignItems = 'flex-start';
        card.innerHTML = `<div style="display:flex; justify-content:space-between; width:100%;"><h3 style="color:var(--ios-blue)">${req.vendorName}</h3><span class="badge" style="background:${req.status === 'Deal Received' ? 'var(--ios-green)' : 'var(--ios-blue)'}">${req.status}</span></div><p style="margin: 5px 0; font-size:14px;">"${req.requirement}"</p><button class="ios-btn-small" style="margin-top:10px;" onclick="openChat('${req.vendorName}')">View Chat History</button>`;
        list.appendChild(card);
    });
}
...
function initMap() {
    ...
    vendors.forEach(v => { if (v.lat && v.lng) L.marker([v.lat, v.lng]).addTo(map).bindPopup(`<b>${v.name}</b><br>${v.category}<br><button class="ios-btn-small" onclick="viewDetails(${v.id}, '94043')">View Deals</button>`); });
    const greenIcon = new L.Icon({ iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-2x-green.png', shadowUrl: 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/0.7.7/images/marker-shadow.png', iconSize: [25, 41], iconAnchor: [12, 41], popupAnchor: [1, -34], shadowSize: [41, 41] });
    projects.forEach(p => { const vendor = vendors.find(v => v.id === p.vendorId); if (p.lat && p.lng) L.marker([p.lat, p.lng], {icon: greenIcon}).addTo(map).bindPopup(`<b>${p.neighborName}'s House</b><br>Active: ${vendor.name}<br><button class="ios-btn-small" onclick="viewDetails(${vendor.id}, '${p.zipcode}')">Join Project</button>`); });
}
...
function shareDeal(vendorName, discount) {
    const text = `Hey neighbors! I'm using HomeGroup to get a deal on ${vendorName}. Join in so we can hit the next tier!`;
    if (navigator.share) navigator.share({ title: 'HomeGroup Deal', text: text, url: window.location.href }); else alert("Simulated Share Sheet:\n\n" + text);
}
